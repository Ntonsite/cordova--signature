...
// WP POSTS RELATED FUNCTIONS
.service('LoanService', function ($rootScope, $http, $q, $ionicLoading, $state, WORDPRESS_LAA_URL, AuthService, BookMarkService, SaveAccountService){
  ...
  this.doAccountOpening = function(account_data) {
    ...
    nonce_dfd.promise.then(function(nonce){
        ...
        $http({
            method:'POST',
            url:WORDPRESS_LAA_URL+'posts/create_account/?callback=JSON_CALLBACK'+'&nonce='+ nonce +'&insecure=cool',
            data: parameters,
            headers : {'Content-Type': 'application/x-www-form-urlencoded'}
        }).then(function(response){
            //success
  ...
  this.getRecentPosts = function(page) {
    ...
    $http.jsonp(WORDPRESS_LAA_URL + 'get_recent_posts/' +
      '?page='+ page +
      '&post_type=account'+
      '&post_author='+JSON.parse(window.localStorage.getItem('ionWordpress_user')).user_id+
      '&callback=JSON_CALLBACK'+
  ...
  this.getApprovedPosts = function(page) {
    ...
    $http.jsonp(WORDPRESS_LAA_URL + 'get_approved_posts/' +
      '?page='+ page +
      '&post_type=account'+
      '&post_status=publish'+
      '&post_author='+JSON.parse(window.localStorage.getItem('ionWordpress_user')).user_id+
  ...
  this.getRejectedPosts = function(page) {
    ...
    $http.jsonp(WORDPRESS_LAA_URL + 'get_rejected_posts/' +
      '?page='+ page +
      '&post_type=account'+
      '&post_status=trash'+
      '&post_author='+JSON.parse(window.localStorage.getItem('ionWordpress_user')).user_id+
  ...
  this.getUserGravatar = function(userId){
    ...
    $http.jsonp(WORDPRESS_LAA_URL + 'user/get_avatar/' +
    '?user_id='+ userId +
    '&type=full' +
    '&callback=JSON_CALLBACK'+
    '&insecure=cool')
  ...
  this.getPost = function(postId) {
    ...
    $http.jsonp(WORDPRESS_LAA_URL + 'get_post/' +
      '?post_id='+ postId +
      '&post_type=account'+
      '&post_author='+JSON.parse(window.localStorage.getItem('ionWordpress_user')).user_id+
      '&callback=JSON_CALLBACK'+
  ...
  this.submitComment = function(postId, content) {
    ...
    $http.jsonp(WORDPRESS_LAA_URL + 'user/post_comment/' +
    '?post_id='+ postId +
    '&cookie='+ user.cookie +
    '&comment_status=1' +
    '&content='+ content +
  ...
  this.getPostsFromCategory = function(categoryId, page) {
    ...
    $http.jsonp(WORDPRESS_LAA_URL + 'get_category_posts/' +
    '?id='+ categoryId +
    '&page='+ page +
    '&callback=JSON_CALLBACK'+
    '&insecure=cool')
  ...
  this.getWordpressPage = function(page_slug) {
    ...
    $http.jsonp(WORDPRESS_LAA_URL + 'get_page/' +
    '?slug='+ page_slug +
    '&callback=JSON_CALLBACK'+
    '&insecure=cool')
    .success(function(data) {
    ...
    nonce_dfd.promise.then(function(nonce){
      ...
      .then(function(data){
        if(data.status == "error"){
          ...
          //receieve and store the user's cookie in the local storage
          var user = {
            cookie: data.cookie,
            data: data.user,
